...
u0=zeros(2*N, 1); %Defining an array to represent all pulses together
...
%u0(1:N) = sech(t*ratio).*sqrt(A); %(1+1i)/sqrt(2)
...
[z, uhat] = ode45(@(z, uhat) SingleRailCoupledPDEs(z,uhat,N,delta,Beta_f1,Beta_f2,Beta_s1,Beta_s2,kappa,gamma,C), z, u0, opts);
...
if (PLOT == true)
    ...
    subplot(1,2,1)         %Plotting F pulse propagation
    pcolor(t,z,abs(u1).^2)
    shading interp
    hold on
    ...
    subplot(1,2,2)         %Plotting S pulse propagation
    pcolor(t,z,abs(u2).^2)
    shading interp
    hold on
...
S = fft(fftshift(u2,2), [], 2);    %Defining a matrix P as the FT of u3 (after shifting u3 to t=0)
S_shift = fftshift(S,2);           %Shifting P so that omega=0 is centred
...
if (PLOT == true)
    ...
    subplot(1,3,1)
    pcolor(freqs,z,abs(S_shift).^2)   %Plotting the FT(P) pulse
    shading interp
    hold on
    ...
    subplot(1,3,2)
    pcolor(freqs,z,real(S_shift))   %Plotting the RE[FT(P)] pulse
    shading interp
    hold on
    ...
    subplot(1,3,3)
    pcolor(freqs,z,imag(S_shift))   %Plotting the Im[FT(P)] pulse
    shading interp
    hold on
    xline(w0, 'w--')   %Plotting a vertical line at 0 to observe the offset of teh frequencies
    hold off
...
[SmaxW,IdxW] = max(abs(S_shift(:)).^2);
[SmaxWRow,SmaxWCol] = ind2sub(size(abs(S_shift).^2),IdxW);  %Finding the maximum of P(z,omega)

if (PLOT == true)
    figure
    subplot(1,2,1)
    plot(freqs, real(S_shift(SmaxWRow,:)))   %Plotting real and imaginary parts of P(z,omega) to observe fine structure
    xlim([w0-freqs_range w0+freqs_range])
    subplot(1,2,2)
    plot(freqs, imag(S_shift(SmaxWRow,:)))
    xlim([w0-freqs_range w0+freqs_range])
end
...
trap = interp1(freqs, interp1(z, S_shift, 0), Wp).*0.5*dz + interp1(freqs, interp1(z, S_shift, zend), Wp).*exp(1i*delta_beta.*zend)*0.5*dz;

for c=1:N-1
    trap = trap + interp1(freqs, interp1(z, S_shift, dz*c), Wp).*exp(1i*delta_beta.*dz*c)*dz; 
end
...
mins = floor((elapsed_time-hours*3600)/60);
secs = rem(elapsed_time, 60);
disp(['Elapsed time: ', num2str(hours), ' hours, ', num2str(mins), ' minutes and ', num2str(secs), ' seconds']);
