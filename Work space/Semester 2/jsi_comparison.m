...
%% Variables to tune
pulsewidth = 10;
C = 0.8; % Units in 1/cm, C=2 corresponds to a rail separation of x=200nm  
A = 0.1; % Amplitude of laser pulse in kiloWatts (kW scaled by constants)
lambda = 745*10^-9;  % Wavelength of photons
...
Beta_p1 = 0; % Test if all pulses have the same beta_1
...
ratio = pulsewidth / (2*sqrt(log(2)));  % FWHM of a Gaussian pulse

u0(1:N) = sqrt(A)*exp(-t.^2 / (2*ratio^2)); % Gaussian pulse
...
max_PW = 2*ratio*sqrt(-2*log(0.5))*D_T*face/(pi*sqrt(A)*10^-12);  % Maximum allowed pulsewidth
max_PA = (D_T*face*ratio*10^12/pi)^2;  % Maximum allowed pulse strength
...
zend = 2.885/C;  % Scaling the length of the waveguide wrt C (linear coupling coefficient)
z = [0:zend/(N-1):zend]'; % Spatial domain in cm (cm determined by constants)
