...
figure            % plot the spectrum
pcolor(fftshift(k),z,abs(fftshift(pspout,2)))
shading interp
xlabel('k')
ylabel('z')
title('pump spectrum UPPE')

figure            % plot the spectrum
pcolor(fftshift(k),z,abs(fftshift(a1spout,2)))
shading interp
xlabel('k')
ylabel('z')
title('A1 spectrum UPPE')

figure            % plot the spectrum
pcolor(fftshift(k),z,abs(fftshift(a2spout,2)))
shading interp
xlabel('k')
ylabel('z')
title('A2 spectrum UPPE')

figure              % plot the field in time domain
pcolor(x,z,abs(Pout_uppe)) 
shading interp
xlabel('t')
ylabel('z')
title('pump UPPE')
...
figure              % plot the field in time domain
pcolor(x,z,abs(A2out_uppe)) 
shading interp
xlabel('t')
ylabel('z')
title('A2 UPPE')

% calculate energy by intergrating in frequency
% use the fact that |psi|=|fft(a)|, i.e. |exp(i beta z)|=1
% plus the fact \int |a|^2 dt = \int |psi|^2 dw
% plus the fact psi(omega)=dft_coeff(omega)*dt/sqrt(2*pi)


ppow_uppe=sum(abs(pspout).^2,2)*dx^2*dk/(2*pi);
a1pow_uppe=sum(abs(a1spout).^2,2)*dx^2*dk/(2*pi);
a2pow_uppe=sum(abs(a2spout).^2,2)*dx^2*dk/(2*pi);
...
function y= rhs_uppe(z,psi,N,betap,beta1,beta2)

propp=exp(1i*betap*z);
prop1=exp(1i*beta1*z);
prop2=exp(1i*beta2*z);

p=ifft(psi(1:N).*propp);
a1=ifft(psi(N+1:2*N).*prop1);
a2=ifft(psi(2*N+1:3*N).*prop2);

po=-1i.*fft(a1.*a2)./propp;
a1o=-1i.*fft(p.*conj(a2))./prop1;
a2o=-1i.*fft(p.*conj(a1))./prop2;

y=vertcat(po,a1o,a2o);
